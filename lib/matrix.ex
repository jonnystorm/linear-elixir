...
defimpl Matrix, for: List do
  def dimension(object = [row|_]) when is_list row do
    {length(object), length(row)}
  end

  def add(object1 = [row1|_], object2 = [row2|_])
      when is_list(row1) and is_list(row2) do
  ...
  def multiply(object1 = [row|_], object2 = [row2|_])
      when is_list(row) and is_list(row2) do
  ...
  def multiply(object = [row|_], row_vector)
      when is_list(row) and is_list(row_vector) do

    multiply object, transpose([row_vector])
  end
  def multiply(object = [row|_], scalar)
      when is_list(row) and is_number(scalar) do
  ...
  def transpose(row_vector) do
    transpose [row_vector]
  end
end
